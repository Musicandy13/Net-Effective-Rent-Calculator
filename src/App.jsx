...
export default function App() {
  ...
  // PNG-Export mit Tenant-Name im Dateinamen
  const exportResultsPNG = async () => {
    if (!pageRef.current) return;
    const tenant = f.tenant?.trim() || "ner-results";
    await exportNode(pageRef.current, `${tenant}-results.png`);
  };
  const exportFullPNG = async () => {
    if (!pageRef.current) return;
    const tenant = f.tenant?.trim() || "ner-full";
    await exportNode(pageRef.current, `${tenant}-full.png`);
  };
  ...
  const exportProjectHTML = () => {
    ...
    const blob = new Blob(
      [
        `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="refresh" content="0; url='${url}'"></head><body></body></html>`,
      ],
      { type: "text/html" }
    );
    const a = document.createElement("a");
  ...
  // Laden von gespeicherten Daten aus URL
  useEffect(() => {
