...
const P = (v) => { // tolerant parse: "1 234,56" → 1234.56
...
const F = (n, d=2) =>
  new Intl.NumberFormat("en-US",{minimumFractionDigits:d,maximumFractionDigits:d}).format(Number.isFinite(n)?n:0);
const FCUR = (n) => (Number.isFinite(n) ? n : 0).toLocaleString("en-US",{style:"currency",currency:"EUR"});
...
function NumericField({
  ...
  format="2dec", step=1, min=0, readOnly=false, onCommit, suffix
}) {
  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        ...
        />
        {suffix && <span className="absolute inset-y-0 right-3 top-1/2 -translate-y-1/2 text-gray-500">{suffix}</span>}
      </div>
    </label>
  );
}
...
export default function App() {
  const [f, setF] = useState({
    nla:"1000", addon:"5.00", rent:"15.00", duration:"60", rf:"7.0",
    agent:"2.0",
    fitMode:"perNLA", fitPerNLA:"150.00", fitPerGLA:"", fitTot:"150000.00",
    unforeseen:"0"
  });
  const S = (k) => (v) => setF(s => ({ ...s, [k]: v }));

  // parsed
  const nla=clamp(P(f.nla)), addon=clamp(P(f.addon)), rent=clamp(P(f.rent)),
        duration=Math.max(0, Math.floor(P(f.duration))), rf=clamp(P(f.rf)),
        agent=clamp(P(f.agent)), unforeseen=clamp(P(f.unforeseen));

  const gla = useMemo(() => nla * (1 + addon/100), [nla, addon]);
  const months = Math.max(0, duration - rf);
  const gross = rent * gla * months;

  // ----- Fit-out modes & conversions -----
  const perNLA = clamp(P(f.fitPerNLA));
  const perGLA = clamp(P(f.fitPerGLA));
  const tot    = clamp(P(f.fitTot));

  // keep fields in sync when inputs change (core conversions)
  useEffect(() => {
    ...
    if (f.fitMode === "perNLA") {
      ...
      if (t !== nTot) S("fitTot")(String(t));
      if (g !== nGLA) S("fitPerGLA")(String(g));
    } else if (f.fitMode === "perGLA") {
      const t = nGLA * gla;
      const n = nla>0 ? t / nla : 0;
      if (t !== nTot) S("fitTot")(String(t));
      if (n !== nNLA) S("fitPerNLA")(String(n));
    } else { // total
  ...
  }, [f.fitMode, f.nla, f.addon]);

  const totalFit =
    f.fitMode === "perNLA" ? perNLA * nla :
    f.fitMode === "perGLA" ? perGLA * gla : tot;
    ...
    <div className="p-6 max-w-xl mx-auto bg-white rounded-xl shadow-md space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <label className="block">
          ...
          <input readOnly value={F(gla,2)}
            className="mt-1 block w-full border rounded-md p-2 bg-gray-100 text-gray-600"/>
        </label>
        <NumericField label="Headline Rent €/sqm" value={f.rent} onChange={S("rent")} format="2dec" step={0.5}/>
        <NumericField label="Lease Term (months)" value={f.duration} onChange={S("duration")} format="int" step={1}/>
        <NumericField label="Rent-Free (months)" value={f.rf} onChange={S("rf")} format="1dec" step={0.5}/>
      </div>
      ...
      <div className="border rounded-md p-3">
        <div className="flex items-center gap-4 mb-3">
          ...
          <label className="inline-flex items-center gap-2">
            <input type="radio" checked={f.fitMode==="perNLA"} onChange={()=>S("fitMode")("perNLA")}/> <span>€/sqm (NLA)</span>
          </label>
          <label className="inline-flex items-center gap-2">
            <input type="radio" checked={f.fitMode==="perGLA"} onChange={()=>S("fitMode")("perGLA")}/> <span>€/sqm (GLA)</span>
          </label>
          <label className="inline-flex items-center gap-2">
        ...
        <NumericField
          ...
          value={f.fitPerNLA}
          onChange={(v)=>{ S("fitPerNLA")(v); if(f.fitMode==="perNLA") S("fitTot")(String(P(v)*nla)); }}
          readOnly={f.fitMode!=="perNLA"} format="2dec" step={1} suffix="€/sqm"
        />
        <NumericField
          label="Fit-Out €/sqm (GLA)"
          value={f.fitPerGLA}
          onChange={(v)=>{ S("fitPerGLA")(v); if(f.fitMode==="perGLA") S("fitTot")(String(P(v)*gla)); }}
          readOnly={f.fitMode!=="perGLA"} format="2dec" step={1} suffix="€/sqm"
        />
        <NumericField
          label="Fit-Out Total (€)"
          value={f.fitTot}
          onChange={(v)=>{ S("fitTot")(v); /* sync happens via useEffect */ }}
          readOnly={f.fitMode!=="total"} format="2dec" step={100} suffix="€"
        />
      </div>

      <div className="grid grid-cols-2 gap-4 mt-4">
        <NumericField label="Agent Fees (months)" value={f.agent} onChange={S("agent")} format="1dec" step={0.5}/>
        <NumericField label="Unforeseen Costs (lump sum €)" value={f.unforeseen} onChange={S("unforeseen")} format="2dec" step={100} suffix="€"/>
      </div>
      ...
      <div className="pt-6 space-y-2 text-left">
        ...
        <p className="border-t pt-2">4️⃣ <b>Final NER</b> (incl. all above + Unforeseen): <b>{F(ner4,2)} €/sqm</b> <Delta base={rent} val={ner4} /></p>
      </div>
    </div>
  );
}
